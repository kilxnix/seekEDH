...
def filter_lands_by_quality(lands, land_quality):
    """
    Enhanced filter to strictly enforce land quality settings

    Args:
        lands (list): List of lands to filter
        land_quality (str): Quality setting ('competitive', 'balanced', or 'budget')

    Returns:
        list: Filtered list of lands
    """
...
def search_cards_by_criteria(query, commander_identity, exclude_cards=None, n=30, price_data=None, max_price=None):
    """
    Enhanced search with stronger price enforcement
    ...
    Returns:
        list: List of cards matching the criteria
    """
    ...
    for i in indices[0]:
        if i < len(card_objects):
            ...
            # Stricter price check - ensure we're actually filtering properly
            if max_price is not None and price_data:
                ...
                # Skip cards with missing price
                if not card_price or card_price == 'null':
                    continue

                try:
                    # Ensure we're converting to float properly
                    price_float = float(card_price)
                    if price_float > max_price:
                        continue
                except (ValueError, TypeError):
                    # If we can't parse the price, skip this card
                    continue
    ...
    # If we didn't get enough results, we can fall back to more lenient searches
    if len(results) < n and max_price is not None:
        logging.info(f"Not enough cards found with price constraint. Got {len(results)}, needed {n}")
